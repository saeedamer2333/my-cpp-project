...
void DataStructureComparator::processArrayStructureSilent()
{
    ...
    // Process Array silently - same operations but no cout output
    arrayCollection.processSilently(transactions, numTransactions, searchKey);

    auto arrayEndTime = chrono::high_resolution_clock::now();

    arrayMetrics.totalTime = chrono::duration_cast<chrono::microseconds>(arrayEndTime - arrayStartTime);
...
void DataStructureComparator::processLinkedListStructureSilent()
{
    ...
    // Process LinkedList silently for performance comparison only
    ...
    linkedListMetrics.totalTime = chrono::duration_cast<chrono::microseconds>(linkedListEndTime - linkedListStartTime);
...
void DataStructureComparator::displayFinalSummary()
{
    ...
    cout << "  Array: " << arrayMetrics.processingTime.count() << " μs" << endl;
    cout << "  LinkedList: " << linkedListMetrics.processingTime.count() << " μs" << endl;
    ...
    cout << "  Array: " << arrayMetrics.sortingTime.count() << " μs" << endl;
    cout << "  LinkedList: " << linkedListMetrics.sortingTime.count() << " μs" << endl;
    ...
    cout << "\nTOTAL PROCESSING TIME:" << endl;
     ...
     // Show in microseconds if less than 1000 microseconds, otherwise milliseconds
    if (arrayMetrics.totalTime.count() < 1000) {
        cout << "Array: " << arrayMetrics.totalTime.count() << " μs" << endl;
    } else {
        cout << "Array: " << (arrayMetrics.totalTime.count() / 1000.0) << " ms" << endl;
    }
    
    if (linkedListMetrics.totalTime.count() < 1000) {
        cout << "LinkedList: " << linkedListMetrics.totalTime.count() << " μs" << endl;
    } else {
        cout << "LinkedList: " << (linkedListMetrics.totalTime.count() / 1000.0) << " ms" << endl;
    }
